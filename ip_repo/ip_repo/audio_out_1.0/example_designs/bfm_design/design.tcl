proc create_ipi_design { offsetfile design_name } {
	...
	connect_bd_intf_net [get_bd_intf_pins master_0/M_AXI_LITE] [get_bd_intf_pins audio_out_0/S00_AXI]

	# Create port connections
	connect_bd_net -net aclk_net [get_bd_ports ACLK] [get_bd_pins master_0/M_AXI_LITE_ACLK] [get_bd_pins audio_out_0/S00_AXI_ACLK]
	connect_bd_net -net aresetn_net [get_bd_ports ARESETN] [get_bd_pins master_0/M_AXI_LITE_ARESETN] [get_bd_pins audio_out_0/S00_AXI_ARESETN]
	...
	set offset_file "${bd_path}/audio_out_v1_0_tb_include.vh"
	set fp [open $offset_file "w"]
	puts $fp "`ifndef audio_out_v1_0_tb_include_vh_"
	puts $fp "`define audio_out_v1_0_tb_include_vh_\n"
...
set ip_path [file dirname [file normalize [get_property XML_FILE_NAME [ipx::get_cores xilinx.com:user:audio_out:1.0]]]]
set test_bench_file ${ip_path}/example_designs/bfm_design/audio_out_v1_0_tb.v
...
for { set i 1 } { 1 } { incr i } {
	set design_name "audio_out_v1_0_bfm_${i}"
...
remove_files -quiet -fileset sim_1 audio_out_v1_0_tb_include.vh
import_files -fileset sim_1 -norecurse -force $interface_address_vh_file
set_property top audio_out_v1_0_tb [get_filesets sim_1]
